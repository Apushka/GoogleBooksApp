    ...
    switch (action.type) {
        case 'GET_CATALOG': {
            return {
                ...
                items: (action.catalog.totalItems && action.catalog.totalItems > 0) ? action.startIndex === 0 ? [...action.catalog.items] : [...state.items, ...action.catalog.items] : []
            }
        }
        case 'IS_FETCHING': {
            return {
...
export const actions = {
    getCatalogAC: (catalog: GetCatalogResponseType, startIndex: number, formData: FormDataType) => ({ type: 'GET_CATALOG', catalog, startIndex, formData } as const),
    isFetchingAC: (isFetching: boolean) => ({ type: 'IS_FETCHING', isFetching } as const)
}
type ActionsTypes = InferActionsTypes<typeof actions>
type ThunkType = BaseThunkType<ActionsTypes>

export const getCatalog = (formData: FormDataType, startIndex: number): ThunkType => async (dispatch, getState) => {
